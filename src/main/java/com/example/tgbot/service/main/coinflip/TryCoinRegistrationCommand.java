...
@Slf4j
public class TryCoinRegistrationCommand implements CommandWithoutCallback {
    ...
    public TryCoinRegistrationCommand(BotActions botActions, UserService userService, String oneWinId, RegistrationService registrationService, UrlService urlService) {
...
//        }
        ...
        if(oneWinId.matches("^\\d{8}$") && user.getAwaitingMode() && registrationService.isPlayerRegistered(oneWinId)) {
            if (userService.isUserExistByOneWinId(Long.parseLong(oneWinId))){
                InlineKeyboardMarkup inlineKeyboardMarkup = CoinFlipKeyboard.menuWithCancelButtonAndLinkToOneWin(urlService.getOneWinUrl(user),user.getLanguage());
                ...
                switch (user.getLanguage()) {
                    ...
                    case "spanish":
                        ...
                        break;
                    case "uzbek":
                        botActions.sendMessageWithInlineKeyboard(chatId, "Siz birovning ID raqamini kiritdingiz!\n" +
...
//            userService.setDefaultAwaitingMode(chatId);
                ...
                CommandWithCallback goToCoinFlipGameCommand = new GoToCoinFlipGameCommand(botActions,userService,urlService);
                goToCoinFlipGameCommand.execute(chatId, new CallbackQuery());
            }
        } else {
            InlineKeyboardMarkup inlineKeyboardMarkup = CoinFlipKeyboard.menuWithCancelButtonAndLinkToOneWin(urlService.getOneWinUrl(user),user.getLanguage());
            ...
            switch (user.getLanguage()) {
                ...
                case "hindi":
                    ...
                    break;
                case "brazilian":
                    botActions.sendMessageWithInlineKeyboard(chatId, "Você digitou uma ID incorreta!\nPor favor, tente novamente.", inlineKeyboardMarkup);
                    break;
                case "spanish":
                ...
                case "azerbaijani":
                    ...
                    break;
                case "turkish":
                    botActions.sendMessageWithInlineKeyboard(chatId, "Yanlış bir kimlik girdiniz!\nLütfen tekrar deneyin.", inlineKeyboardMarkup);
                    break;
                case "portuguese":
                    botActions.sendMessageWithInlineKeyboard(chatId, "Você digitou uma ID incorreta!\nPor favor, tente novamente.", inlineKeyboardMarkup);
                    break;
